...
const Card = ({ username: id, type, name: title, description, reviews, avatar: image, category, classes = "", isFetching = false, error = null }) => {
            ...
            <div className='flex px-6 md:px-4 py-2'>
                <div className='flex flex-col items-center'>
                    <div className='w-[50px] sm:w-[60px] md:w-[70px] aspect-square overflow-hidden rounded-full my-2'>
                        {isFetching ? <div className="skeleton h-full w-20"></div> : <Link to={`/${type}/${id}`}><img
                            src={image}
                            alt=""
                ...
                <div className='ml-4 h-[5rem]'>
                    {isFetching ? <div className="skeleton h-4 w-20 my-2"></div> : <Link to={`/${type}/${id}`}><h1 className='font-semibold text-sm md:text-md'>{title}</h1></Link>}
